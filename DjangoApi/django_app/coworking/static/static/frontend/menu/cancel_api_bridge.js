// cancel_api_bridge.js — Cancel via API: find by ID or Seat+Date; emit events; update UI
(function(){
  ...
  async function apiCheck(seat, date){
    ...
    return await res.json(); // {reserved:bool, reservation_id: str|null, ...}
  }
  ...
  async function searchAndRender(){
    ...
    if(id){
      // Show single row by id (we don't have GET /.../{id} -> list by date range instead if date provided)
    ...
    if(seat && date){
      ...
      if(chk && chk.reserved && chk.reservation_id){
        ...
        if(items.length){
          ...
          // fallback: render pseudo-row z ID z /check
    ...
    // If only date provided -> list that day
    if(date && !seat){
    ...
    // If only seat provided -> list seat in near range (±30 dni)
    if(seat && !date){
      const now = new Date();
  ...
  // Submit of the cancel form: try to resolve and delete immediately
    ...
    try{
      if(!targetId){
        if(seat && date){
          ...
          // no direct target -> show results instead of deleting blindly
  ...
  // Click on dynamically listed cancel buttons
  ...
  // Optional: provide a "Find" button if page has it
