// reserved_sync.js — trwała literka R: klasa .seat-r + localStorage + parę retry
(function () {
  ...
  // Jednorazowe odtworzenie stanu (kilka prób, żeby wygrać z lockSeats())
  ...
  // Reakcje na tworzenie/anulowanie (te eventy już emitujesz w swoim froncie)
  ...
  window.addEventListener("reservation:deleted", (e) => {
    const d = e.detail || {};
    const seat = d.seat_id || d.seat || d.place || d.seatId || d.id_seat;
    // jeśli event nie niesie seatId (często tak bywa) — nie ruszamy zbioru
    if (!seat) return;
    const set = loadSet(); set.delete(String(seat)); saveSet(set);
    removeClassForSeat(seat);
  });

  // start — kilka prób po wejściu/odświeżeniu, żeby nałożyć klasę po tym jak script.js zbuduje kafelki
  if (document.readyState === "complete") scheduleRenderAll();
  else window.addEventListener("load", scheduleRenderAll);
  // zabezpieczenie dla Back/forward cache
