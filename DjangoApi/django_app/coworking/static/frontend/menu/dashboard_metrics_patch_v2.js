// dashboard_metrics_patch_v2.js
// Live refresh for: "Reservations (last 14 days)" chart + "My reservations" + Summary widgets.
// Works with your existing FullCalendar & API. Non‑breaking: only enhances if elements exist.

(function(){
  const token = localStorage.getItem('token') || '';
  const auth = token ? { Authorization: 'Bearer '+token } : {};

  // ---- DOM refs (fallback-friendly) ----
  ...
  // Chart canvas (try multiple selectors)
  const chartCanvas =
  ...
  // "My reservations" list container (try multiple ids)
  const myList =
  ...
  // ---- "My reservations" renderer ----
  function renderMyReservations(items){
    if(!myList) return;
    // Upcoming first (>= today), sorted asc; then past (desc), keep total <= 12 rows
  ...
  // ---- Master refresh (single API call reused for all sections) ----
