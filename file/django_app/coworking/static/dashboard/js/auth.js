    ...
    loginForm.addEventListener('submit', async function(e) {
      ...
      if (useMagicLink) {
        ...
        try {
          ...
          if (!response.ok) {
            throw new Error(data.detail || data.error || window.t?.('Failed to send magic link') || 'Failed to send magic link');
          }
      ...
      try {
        ...
        const data = await response.json();
          ...
          if (!response.ok) {
            throw new Error(data.detail || data.message || window.t?.('Login failed') || 'Login failed');
        }
    ...
    if (verify2FABtn) {
      verify2FABtn.addEventListener('click', async function() {
        ...
        try {
          ...
          if (!response.ok) {
            throw new Error(data.detail || window.t?.('Verification failed') || 'Verification failed');
          }
  ...
  if (registerForm) {
    registerForm.addEventListener('submit', async function(e) {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(data.detail || data.message || window.t?.('Registration failed') || 'Registration failed');
        }
  ...
  if (magicLinkForm) {
    magicLinkForm.addEventListener('submit', async function(e) {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(data.detail || window.t?.('Failed to send magic link') || 'Failed to send magic link');
        }
