 ...
 (function(){
  ...
  async function refreshSummary(items){
    try{
      ...
      if(elNext)  elNext.textContent  = next ? `${next.date} (${translate('seat', 'stanowisko')} ${next.seat_id})` : '—';
      if(elLast)  elLast.textContent  = last ? `${last.date} (${translate('seat', 'stanowisko')} ${last.seat_id})` : '—';
      ...
      if(elIcs){
        elIcs.onclick = (e)=>{
          ...
          const ics = [
            ...
            `DTEND;VALUE=DATE:${dt}`,
            `SUMMARY:${translate('Seat', 'Stanowisko')} ${next.seat_id}`,
            `DESCRIPTION:${translate('Reservation', 'Rezerwacja')} ${next.id}`,
            `LOCATION:${translate('Seat', 'Stanowisko')} ${next.seat_id}`,
  ...
  function renderMyReservations(items){
    ...
    myList.innerHTML = '';
    if(!rows.length){
      myList.innerHTML = `<li class="muted">${translate('No reservations yet.', 'Brak rezerwacji.')}</li>`;
      return;
    }
    for(const r of rows){
      ...
      li.innerHTML = `<span>${r.date}</span> • <strong>${r.seat_id}</strong> <small>(ID ${r.id})</small>
                      <button class="mini-cancel" data-cancel-id="${r.id}" style="margin-left:8px">${translate('Cancel', 'Anuluj')}</button>`;
    ...
    try{
      ...
      if(!chart){
        chart = new Chart(chartCanvas.getContext('2d'), {
          type: 'bar',
          data: { labels, datasets: [{ label: translate('Reservations', 'Rezerwacje'), data: counts }] },
          options: {
            responsive: true,
            animation: false,
        ...
        chart.data.datasets[0].label = translate('Reservations', 'Rezerwacje');
        chart.update();
      }
    }catch(err){
