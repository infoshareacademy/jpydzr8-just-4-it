...
function updateFloorOccupancy() {
  floors.forEach(floor => {
    const grid = document.getElementById(floor.id);
    const target = document.querySelector(`.seat-occupancy[data-floor="${floor.floor}"]`);
    if (!grid || !target) return;
    const seats = Array.from(grid.querySelectorAll('.seat-item-styled'));
    if (!seats.length) {
      target.textContent = '';
      return;
    }
    const reserved = seats.filter(el => el.dataset.status === 'reserved').length;
    const free = seats.length - reserved;
    const labelFree = target.dataset.labelFree || 'Free';
    const labelOcc = target.dataset.labelOccupied || 'Occupied';
    target.innerHTML = `
      <span class="pill"><span class="swatch free"></span>${free} ${labelFree}</span>
      <span class="pill"><span class="swatch occupied"></span>${reserved} ${labelOcc}</span>
    `;
  });
}
...
(function(){
  ...
  function lockSeats(){
    ...
    if (typeof updateFloorOccupancy === 'function') {
      updateFloorOccupancy();
    }
  }
