...
async function updateAvailabilitySVG(){ 
  const root=document.getElementById('floor-root'); 
  if(!root) return; 
  const floor=root.dataset.floor; 
  const date=document.getElementById('pref_date')?.value||''; 
  const from=document.getElementById('pref_from')?.value||'09:00'; 
  const to=document.getElementById('pref_to')?.value||'17:00'; 
  const q=new URLSearchParams({date,from,to}).toString(); 
  try {
    const res=await fetch(`/reservations/floor/${floor}/availability/?`+q); 
    if (!res.ok) {
      console.warn('Availability endpoint returned error:', res.status);
      return;
    }
    const contentType = res.headers.get('content-type');
    if (!contentType || !contentType.includes('application/json')) {
      console.warn('Availability endpoint did not return JSON');
      return;
    }
    const data=await res.json(); 
    document.querySelectorAll('svg .desk-dot').forEach(el=>el.classList.remove('free','busy')); 
    (data.available||[]).forEach(id=>{ 
      const marker=document.querySelector(`[data-id="${id}"]`); 
      const dot=marker?.querySelector('.desk-dot'); 
      if(dot) dot.classList.add('free'); 
    }); 
    ...
    if (typeof toast === 'function') toast('Zajętość uaktualniona'); 
  } catch (error) {
    console.warn('Error fetching availability:', error);
  }
}
document.addEventListener('DOMContentLoaded',()=>{ 
  document.getElementById('btn-availability')?.addEventListener('click', updateAvailabilitySVG); 
  // Only auto-update if we're on a floor page
  if (document.getElementById('floor-root')) {
    setTimeout(updateAvailabilitySVG, 300); 
  }
});
