...
def admin_control_center(request):
    """New admin cockpit with user focused insights"""
    User = get_user_model()
    now = timezone.now()
    ...
    new_users = User.objects.filter(**{f"{join_field}__gte": week_ago}).count()

    recent_signups_qs = User.objects.order_by(f'-{join_field}')[:6]
    recent_signups = [
        {
            'email': user.email,
            'joined': getattr(user, join_field, None),
            'is_active': user.is_active,
        }
        for user in recent_signups_qs
    ]

    recent_activity_qs = User.objects.filter(last_login__isnull=False).order_by('-last_login')[:6]
    recent_activity = [
        {
            'email': user.email,
        ...
        for user in recent_activity_qs
    ]

    today = now.date()
    reservations_today = Reservation.objects.filter(date=today, is_cancelled=False).count()
    reservations_week = Reservation.objects.filter(date__gte=today - timedelta(days=7), is_cancelled=False).count()
    cancellations_week = Reservation.objects.filter(date__gte=today - timedelta(days=7), is_cancelled=True).count()

    dashboard_overview = {}
    performance_metrics = {}
    try:
        dashboard_data = AnalyticsEngine.get_dashboard_data(7)
        dashboard_overview = dashboard_data.get('overview', {})
    except Exception:
        dashboard_data = {}
    try:
    ...
    performance_label_map = {
        'avgresponsetime': _("Average response time (ms)"),
        'avg_response_time': _("Average response time (ms)"),
        'avg_response_time_ms': _("Average response time (ms)"),
        'dbsizemb': _("Database size (MB)"),
        'db_size_mb': _("Database size (MB)"),
        'activesessions': _("Active sessions"),
        'active_sessions': _("Active sessions"),
        'errorrate': _("Error rate"),
        'error_rate': _("Error rate"),
        'uptimepercentage': _("Uptime percentage"),
        'uptime_percentage': _("Uptime percentage"),
    }

    performance_metrics_display = []
    for key, value in performance_metrics.items():
        normalized_key = key.lower().replace(' ', '').replace('-', '')
        label = performance_label_map.get(normalized_key, key.replace('_', ' ').title())
        performance_metrics_display.append({
            'key': key,
            'label': label,
            'value': value,
        })
    ...
    context = {
        ...
        'reservation_metrics': {
            ...
            'cancellations': cancellations_week,
        },
        'dashboard_overview': dashboard_overview,
        'performance_metrics': performance_metrics,
        'performance_metrics_display': performance_metrics_display,
