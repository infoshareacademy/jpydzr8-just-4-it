...
def home(request):
    """Przekierowanie do domyślnego piętra (4) lub pierwszego dostępnego"""
    try:
        # Sprawdź czy piętro 4 istnieje
        Floor.objects.get(number=4)
        return redirect('floor', floor_number=4)
    except Floor.DoesNotExist:
        # Jeśli nie ma piętra 4, przekieruj do pierwszego dostępnego
        first_floor = Floor.objects.order_by('number').first()
        if first_floor:
            return redirect('floor', floor_number=first_floor.number)
        # Jeśli nie ma żadnych pięter, zwróć komunikat
        from django.http import HttpResponse
        return HttpResponse(
            "<h1>Brak danych w bazie</h1>"
            "<p>Uruchom migracje aby utworzyć podstawowe dane:</p>"
...
def reservation_ics(request, res_id):
    """Download ICS file for a reservation. Supports both old and new reservation models.
    
    res_id can be:
    - Integer: Old API model (for backward compatibility)
    - 'res_<id>': New reservations.models.Reservation
    - 'api_<id>': Old api.models.Reservation (explicit)
    """
    from api.models import Reservation as ApiReservation
    
    if isinstance(res_id, str):
        if res_id.startswith('res_'):
            actual_id = int(res_id[4:]) if res_id[4:].isdigit() else res_id[4:]
            res = get_object_or_404(Reservation, id=actual_id)
        elif res_id.startswith('api_'):
            actual_id = res_id[4:]
            res = get_object_or_404(ApiReservation, id=actual_id)
        else:
            try:
                actual_id = int(res_id)
                try:
                    res = Reservation.objects.get(id=actual_id)
                except Reservation.DoesNotExist:
                    res = get_object_or_404(ApiReservation, id=res_id)
            except ValueError:
                try:
    ...
    has_desk = hasattr(res, 'desk') and res.desk is not None
    has_seat_id = hasattr(res, 'seat_id') and not has_desk
    
    if has_desk:
        dtstart = _ics_datetime(res.date, res.time_from)
        dtend = _ics_datetime(res.date, res.time_to)
        summary = f"Rezerwacja: {res.desk.label}"
        location = f"Piętro {res.desk.floor.number}"
        description = f"Stanowisko: {res.desk.label}"
        if res.name:
            description += f"\\nRezerwujący: {res.name}"
        if res.email:
            description += f"\\nEmail: {res.email}"
    elif has_seat_id:
        from datetime import timedelta
        dtstart = _ics_datetime_old(res.date)
        try:
            dt = datetime.strptime(res.date, '%Y-%m-%d') + timedelta(days=1)
            dtend = dt.strftime('%Y%m%d')
        except (ValueError, AttributeError, TypeError):
            dtend = (datetime.utcnow() + timedelta(days=1)).strftime('%Y%m%d')
        summary = f"Rezerwacja: {res.seat_id}"
        location = "Just 4 IT Coworking"
        description = f"Stanowisko: {res.seat_id}"
        if res.name:
            description += f"\\nRezerwujący: {res.name}"
        if res.email:
            description += f"\\nEmail: {res.email}"
    else:
        if res.__class__.__module__.startswith('reservations.models'):
            try:
                dtstart = _ics_datetime(res.date, res.time_from)
                dtend = _ics_datetime(res.date, res.time_to)
            except (AttributeError, TypeError):
                dtstart = res.date.strftime('%Y%m%d') if hasattr(res.date, 'strftime') else _ics_datetime_old(str(res.date))
                dtend = (res.date + timedelta(days=1)).strftime('%Y%m%d') if hasattr(res.date, 'strftime') else dtstart
            summary = f"Rezerwacja: {getattr(res.desk, 'label', 'N/A')}"
            location = f"Piętro {getattr(res.desk.floor, 'number', 'N/A')}" if hasattr(res, 'desk') and res.desk else "N/A"
            description = f"Stanowisko: {getattr(res.desk, 'label', 'N/A')}"
        else:
            from datetime import timedelta
            dtstart = _ics_datetime_old(res.date)
            try:
                ...
                dtend = (datetime.utcnow() + timedelta(days=1)).strftime('%Y%m%d')
            summary = f"Rezerwacja: {getattr(res, 'seat_id', 'N/A')}"
            location = "Just 4 IT Coworking"
            description = f"Stanowisko: {getattr(res, 'seat_id', 'N/A')}"
    
    lines = [
        "BEGIN:VCALENDAR",
        "VERSION:2.0",
        "PRODID:-//Office Reservation//EN",
        "CALSCALE:GREGORIAN",
        "METHOD:PUBLISH",
        "BEGIN:VEVENT",
        f"UID:{res.id}@office.local",
...
def _ics_datetime_old(date_str: str) -> str:
    """Convert date string (YYYY-MM-DD) to ICS datetime format for all-day events"""
    try:
...
def select_desk_for_recurring(request):
    """Wybór biurka do utworzenia rezerwacji cyklicznej"""
    from reservations.models import Floor, Desk
    
    floors = Floor.objects.all().order_by('number')
    desks = Desk.objects.select_related('floor', 'zone').order_by('floor__number', 'label')
    
    return render(request, 'reservations/select_desk_recurring.html', {
        'floors': floors,
        'desks': desks
    })
...
def recurring_reservations(request):
    ...
    recurring = RecurringReservation.objects.filter(
        email=email, 
        is_active=True
    ).select_related('desk', 'desk__floor').order_by('-created_at')
...
def create_recurring_reservation(request, desk_id: int):
    ...
    if request.method == 'POST':
        logger.info(f'POST request do create_recurring_reservation: desk_id={desk_id}, POST={dict(request.POST)}')
        form = RecurringReservationForm(request.POST, user=request.user)
        if form.is_valid():
            logger.info(f'Formularz ważny, zapisuję recurring reservation dla desk={desk_id}')
            try:
                ...
                # If user is authenticated, use their email
                if request.user.is_authenticated:
                    recurring.email = getattr(request.user, 'email', recurring.email)
                    if not recurring.name:
                        recurring.name = getattr(request.user, 'full_name', '') or recurring.email.split('@')[0]
                
                recurring.save()
                logger.info(f'Recurring reservation zapisany: id={recurring.id}, desk={desk.label}, email={recurring.email}')
                
                # Generuj pierwsze rezerwacje
                created_count = generate_recurring_reservations()
                logger.info(f'Wygenerowano {created_count} rezerwacji z cyklu')
                
                # Wyślij potwierdzenie konfiguracji cyklicznej rezerwacji
                try:
                    from .notification_utils import send_recurring_setup_confirmation
                    # Spróbuj znaleźć pierwszą świeżo utworzoną rezerwację tego cyklu (opcjonalnie)
                    first_res = Reservation.objects.filter(
                        desk=recurring.desk,
                        email=recurring.email,
                        date__gte=recurring.start_date,
                        is_cancelled=False
                    ).order_by('date', 'time_from').first()
                    send_recurring_setup_confirmation(recurring, first_res)
                except Exception:
                    logger.exception("Błąd wysyłki potwierdzenia konfiguracji cyklicznej rezerwacji: recurring_id=%s", recurring.id)
                ...
                logger.exception(f'Błąd podczas tworzenia cyklicznej rezerwacji: {e}')
                form.add_error(None, f'Błąd podczas zapisywania: {str(e)}')
            ...
            logger.warning(f'Formularz nieważny: {form.errors}')
            logger.warning(f'POST data: {dict(request.POST)}')
            # Re-render form with errors
            return render(request, 'reservations/create_recurring.html', {
                'form': form,
                'desk': desk,
                'debug_errors': form.errors  # Add for debugging
            })
    else:
        # GET request - show form
        form = RecurringReservationForm(user=request.user)
        logger.info(f'GET request do create_recurring_reservation: desk_id={desk_id}')
...
def cancel_recurring(request, recurring_id: int):
    ...
    if request.method == 'POST':
        ...
        recurring.save()
        # Wyślij mail o anulowaniu (nie blokuj w razie błędu)
        try:
            from .notification_utils import send_recurring_cancellation
            send_recurring_cancellation(recurring)
        except Exception:
            logger.exception("Błąd wysyłki powiadomienia o anulowaniu cyklicznej rezerwacji: recurring_id=%s", recurring.id)
        return JsonResponse({
            'success': True,
            'message': f'Cykliczna rezerwacja dla {recurring.desk.label} została anulowana'
        })
...
def notification_settings(request):
    ...
    if email:
        try:
            ...
            # Auto-create default preferences for convenience
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if group_reservation:
                # Wyślij email do organizatora (nie blokuj sukcesu w razie błędu)
                try:
                    from .notification_utils import send_group_reservation_confirmation
                    send_group_reservation_confirmation(group_reservation)
                except Exception:
                    logger.exception("Błąd wysyłki potwierdzenia rezerwacji grupowej: group_id=%s", group_reservation.id)
...
def conference_room_reservation(request, room_id: int):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Sprawdź konflikty czasowe przed zapisem
            from django.db.models import Q
            conflicts = ConferenceReservation.objects.filter(
                conference_room=conference_room,
                date=reservation.date,
                is_cancelled=False
            ).filter(
                Q(time_from__lt=reservation.time_to) & Q(time_to__gt=reservation.time_from)
            ).exists()
            ...
            if conflicts:
                return render(request, 'reservations/conference_room_reservation.html', {
                    'form': form,
                    'conference_room': conference_room,
                    'error': 'Ten termin jest już zajęty dla wybranej sali. Wybierz inny czas.'
                })
            
            # Zapis z obsługą wyjątków unikalności
            try:
                reservation.save()
            except Exception as e:
                ...
                if isinstance(e, IntegrityError):
                    return render(request, 'reservations/conference_room_reservation.html', {
                        'form': form,
                        'conference_room': conference_room,
                        'error': 'Wybrany dokładny termin jest już zarezerwowany. Spróbuj innego zakresu godzin.'
                    })
                raise
            
            # Wyślij potwierdzenie rezerwacji sali (nie blokuj w razie błędu)
            try:
                from .notification_utils import send_conference_reservation_confirmation
                send_conference_reservation_confirmation(reservation)
            except Exception:
                logger.exception("Błąd wysyłki potwierdzenia rezerwacji sali konferencyjnej: reservation_id=%s", reservation.id)
