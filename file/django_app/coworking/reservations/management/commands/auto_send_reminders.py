...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS(f'Starting automatic reminder system (interval: {interval}s)'))
        ...
        try:
            while True:
                try:
                    ...
                    if sent_count > 0:
                        self.stdout.write(self.style.SUCCESS(f'[{datetime.now().strftime("%H:%M:%S")}] Sent {sent_count} reminder(s)'))
                    else:
                        self.stdout.write(
            ...
            self.stdout.write(self.style.SUCCESS('Automatic reminder system stopped'))
