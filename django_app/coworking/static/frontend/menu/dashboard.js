(function(){
  ...
  function renderUpcoming(){
    ...
    if(list.length===0){ body.classList.add('empty'); body.textContent = 'You have no upcoming reservations.'; return; }
    body.classList.remove('empty');
    const r = list[0];
    body.innerHTML = `<strong>Seat ${r.seat}</strong> — <span>${r.date}</span><br><small>ID: ${r.id}</small>`;
  }
  ...
  function renderRecent(){
    ...
    const list = parseReservations()
      ...
      .slice(0,5);
    if(list.length===0){
      cont.innerHTML = '<div class="recent-item"><i class="fas fa-info-circle"></i><div class="recent-info">No recent reservations.</div></div>';
      return;
    }
    list.forEach(r=>{
      ...
      item.innerHTML = `<i class="fas fa-calendar-day"></i>
        <div class="recent-info"><strong>Seat ${r.seat}</strong> — <span>${r.date}</span><br><small>ID: ${r.id}</small></div>`;
